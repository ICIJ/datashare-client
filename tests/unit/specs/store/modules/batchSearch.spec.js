...
describe('BatchSearchStore', () => {
  ...
  describe('actions', () => {
    ...
    it('should retrieve all the batchSearches', async () => {
      ...
      const data = { from: 0, size: 10, sort: 'batch_date', order: 'asc', query: '*', field: 'all', batchDate: [], project: [] }
    ...
    it('should submit the new batchSearch form with complete information', async () => {
      ...
      expect(axios.request).toBeCalledWith(expect.objectContaining({
        data: { from: 0, size: 100, sort: 'batch_date', order: 'asc', query: '*', field: 'all', batchDate: null, project: [] },
        method: 'POST',
        url: Api.getFullUrl('/api/batch/search')
      }))
    })
