...
describe('FormControlPath.vue', () => {
  ...
  describe('without path', () => {
    ...
    beforeEach(() => {
      ...
      wrapper = mount(FormControlPath, { global: { plugins } })
    })

    it('should render the directory list with only "Home"', () => {
      const directories = wrapper.findAll('.form-control-path__header__list__item')
    ...
    it('should render the "Home" tree when clicking on "Browse"', async () => {
      ...
      wrapper.find('.form-control-path__header__browse').trigger('click')
      await flushPromises()
      const subDirectories = wrapper.findAll('.form-control-path__browser__item__link')
    ...
    it('should show a "Close" label on the browsing when browsing', async () => {
      api.tree.mockResolvedValue(HOME_TREE)
      const btn = wrapper.find('.form-control-path__header__browse')
    ...
    it('should enter the "01FOO" tree when clicking on the sub-directory', async () => {
      ...
      wrapper.findAll('.form-control-path__browser__item__link').at(0).trigger('click')
      await flushPromises()
  ...
  describe('with path "/home/dev/Datashare/01FOO"', () => {
    ...
    beforeEach(() => {
      ...
      const { plugins, config } = CoreSetup.init(api).useAll()
      config.set('dataDir', '/home/dev/Datashare/')
      wrapper = mount(FormControlPath, { global: { plugins }, props })
      api.tree.mockResolvedValue(HOME_01FOO_TREE)
      config.set('dataDir', '/home/dev/Datashare/')
    })
    ...
    it('should emit an input event when clicking on "Home"', async () => {
      wrapper.findAll('.form-control-path__header__list__item').at(0).trigger('click')
      await flushPromises()
      expect(wrapper.emitted()).toHaveProperty('update:path')
      expect(wrapper.emitted('update:path')).toHaveLength(1)
