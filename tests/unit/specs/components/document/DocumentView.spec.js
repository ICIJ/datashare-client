...
describe('DocumentView.vue', () => {
  ...
  it('should display named entities in the dedicated tab', async () => {
    ...
    expect(wrapped.vm.$el.querySelectorAll('.facet--named-entity .facet__items__item__key').length).toEqual(3)
    expect(trim(wrapped.vm.$el.querySelectorAll('.facet--named-entity .facet__items__item__key')[0].textContent)).toEqual('mention_01')
    expect(wrapped.vm.$el.querySelectorAll('.facet--named-entity .facet__items__item__key')[0].classList.contains('bg-category-category_01')).toEqual(true)
    expect(trim(wrapped.vm.$el.querySelectorAll('.facet--named-entity .facet__items__item__key')[1].textContent)).toEqual('mention_02')
    expect(wrapped.vm.$el.querySelectorAll('.facet--named-entity .facet__items__item__key')[1].classList.contains('bg-category-category_02')).toEqual(true)
    expect(trim(wrapped.vm.$el.querySelectorAll('.facet--named-entity .facet__items__item__key')[2].textContent)).toEqual('mention_03')
    expect(wrapped.vm.$el.querySelectorAll('.facet--named-entity .facet__items__item__key')[2].classList.contains('bg-category-category_03')).toEqual(true)
  })
})
