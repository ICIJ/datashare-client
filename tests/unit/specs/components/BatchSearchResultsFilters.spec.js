...
describe('BatchSearchResultsFilters.vue', () => {
  ...
  it('should display simple list if there is only one query', async () => {
    await store.commit('batchSearch/batchSearch', batchSearches['13'])
    wrapper = mount(BatchSearchResultsFilters, {
      ...
      propsData: { uuid: '13', indices: project }
    })
  ...
  it('should display a selectable dropdown if there are more than one query', async () => {
    await store.commit('batchSearch/batchSearch', batchSearches['12'])
    wrapper = mount(BatchSearchResultsFilters, {
      i18n,
      localVue,
      router,
      store,
      computed: { downloadLink () { return 'mocked-download-link' } },
      propsData: { uuid: '12', indices: [{ name: project }, { name: anotherProject }] }
    })
  ...
  it('should add badge with query number of results on selectable dropdown', async () => {
    ...
    wrapper = mount(BatchSearchResultsFilters, {
      ...
      store,
      computed: { downloadLink () { return 'mocked-download-link' } },
      propsData: { uuid: '12', indices: [project, anotherProject] }
    })
