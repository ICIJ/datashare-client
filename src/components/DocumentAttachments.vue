  ...
  <div class="document-attachments">
    ...
    <ul class="document-attachments__list list-unstyled d-flex-inline" v-if="isReady">
      <li v-for="attachment in attachments" class="document-attachments__list__item" :key="attachment.id">
        <router-link :to="{ name: 'document', params: attachment.routerParams }" class="document-attachments__list__item__link">
          <fa :icon="attachment.contentTypeIcon" fixed-width class="mr-1" />
          {{ attachment.slicedName.pop() }}
        </router-link>
      </li>
    </ul>
