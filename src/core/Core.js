...
class Core extends Behaviors {
  ...
  constructor(api = apiInstance) {
    super()
    const components = { BApp }
    const template = '<b-app><router-view /></b-app>'
    const Root = defineComponent({ components, template })
  ...
  applyPageTitle() {
    if (window.document) {
      window.document.title = this.pageTitle ? `${this.pageTitle} - Datashare` : 'Datashare'
    }
    return this
  }
